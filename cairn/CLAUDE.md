...
| `CAIRN_VISUAL_ROADMAP.md` (+ its `.html` twin) | ~60k tok | never — read the **newest block only**, which supersedes the ones below it (revision 55's block is the newest and it corrects the `I-13f` claim the two below it got wrong) | not a contract doc; skip it for a routine builder/breaker task, and see below for when to *update* it |
