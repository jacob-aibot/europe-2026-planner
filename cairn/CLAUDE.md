...
| `ROADMAP.md` | **~156k tok** | **never** | `cairn/tools/doc-section ROADMAP "Phase N" "Sequencing rules"` (matches heading text as well as number — see the tool's `--help`). Phase 2 alone is **~111k**; grep for your increment. **Before ordering Phase 2's gate, grep `I-11 — The phase gate` and read its *Dependencies / blockers* bullet whole — revision 54 made it the authoritative built/not-built list, and it supersedes the 2d status cell and the visual roadmap where they disagree** |
...
| `CAIRN_VISUAL_ROADMAP.md` (+ its `.html` twin) | ~58k tok | never — read the **newest block only**, which supersedes the ones below it | not a contract doc; skip it for a routine builder/breaker task, and see below for when to *update* it |
