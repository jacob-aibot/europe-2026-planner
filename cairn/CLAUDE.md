...
**`ARCHITECTURE.md` and `ROADMAP.md` were re-measured at revision 41 (2026-09-03); the rest date from
...
| `ARCHITECTURE.md` | **~318k tok** | **never.** Not even for a broad design revision — read sections | `cairn/tools/doc-section ARCHITECTURE <sections>`; run it bare to list every section with its size, and see the table at the top of the doc for who needs what. §2 is 123k, §8 is **77k** (revision 41: A-59/A-60 joined §8.4), §4 is 60k; **§10 (photos, revision 40) is 13k and self-contained** |
| `ROADMAP.md` | **~114k tok** | **never** | `cairn/tools/doc-section ROADMAP "Phase N" "Sequencing rules"` (matches heading text as well as number — see the tool's `--help`). Phase 2 alone is ~75k; grep for your increment |
