...
§7 ~2k; **§1, §2, §3, §4, §8, §9, §10 and §11 do not move at all**), and `ROADMAP` 268k → **274k** with the
rewritten **Phase 3** (11k → **6k** as `doc-section` reports it — the section got *shorter* and five times
more specific), **sequencing rule 14**, the revision-80 ledger entry and one corrected row in the phase
table (Phase 2 is unmoved at **192k**). **§12 is ~11k and it is SELF-CONTAINED and READ ALONE — a builder or
breaker of `I-39`…`I-43` reads §12 whole and NOTHING ELSE in `ARCHITECTURE.md`**: not §2, not §4, not §8,
not §10, not §11, and no `A-` ruling at all. **Its Part 3 (the never-list) and Part 4 (what a share page may
contain) are read before any other Phase 3 decision, and before anyone buys a host, a database or an auth
provider.** This revision is **NOT a QA consequence**: Jacob directed that Phase 3 begin, and this is the
design pass that must precede infrastructure. **Nothing has been purchased** and §12 Part 11 names the only
two decisions that genuinely depend on a vendor. **The scope of Phase 3 moves, which is why the visual
roadmap moves with it**: the phase ships **`owner` and `viewer`** and does **not** mint `editor`,
`commenter` or `TripMember` — the CRDT trigger is restated as *a `MergeReport` with a reader who is not the
writer*, and all three roles stay in the predicates and in every cell of the conformance matrix. **No code
file, no `.tsx`, no `apps/web`, no `qa/`, no `docs/design/`, no golden byte, no version constant — §2.10
stays at 91 — and nothing here gates `I-30`**, which is still fenced by the unresolved visual direction and
by nothing else. **Two rows moved with QA round 72's own commit rather than with this revision**:
`QA-FINDINGS` 431k → **436k** and `BUILD-NOTES` 335k → **337k**. `CAIRN_VISUAL_ROADMAP` re-measures at
**75k** (`.html` **89k**) **before** this revision's block and moves with it; `REVIEW` is unmoved at **52k**.
...
**189k**). **A-96 is ~5k and it is the ENTRY POINT to §11 — read it first, then §11.4, §11.5, §11.7 and
§11.8, each of which carries its amendment banner in place.** A builder or breaker of `I-37` reads **§11
whole and nothing else in `ARCHITECTURE.md`**. This revision is QA round 70's architect half, and it is
**one ruling with two faces and a correction**: `free_time` shipped *"Cairn does not invent a duration for a
stop that states none"* **and also refused to read the duration a stop does state** in `arrival.mins`
(**21 of 21** journey stops on the reference trip, all with `durationMins: null`) — the computation already
existed, module-private, in `conflict/rules/overlap.ts`, so this was a second opinion rather than an
oversight, and it moves to **`derive/occupancy.ts`** with both callers reading it; and `answer.text`
interpolated `trip.title` and `City.name` verbatim, so §11.8's redaction claim was a property of the
fixture's strings — **a record's free text may be narrated only where the closed `Question` union makes
that record the SUBJECT of the question**, which admits exactly `City.name`, guarded. The correction is
**§11.7 rule 4**, whose two-city gate would print *"no stop on that day is recorded as a journey"* about two
of twelve edges that record one; **the builder was right and the code is not touched** (KD-131).
**No export, no version constant, no golden byte, no `.tsx`, no `apps/web`, no corpus — §2.10 stays at 91 —
and nothing here gates `I-30`.** **Two rows moved with round 70's own commit rather than with this
revision**: `QA-FINDINGS` 410k → **425k** and `BUILD-NOTES` 311k → **333k** — re-measure before quoting the
rows below. `CAIRN_VISUAL_ROADMAP` is unmoved at 74k (this revision opens no rendered surface) and `REVIEW`
at 52k.

**Every row re-measured at revision 73/76 (2026-09-10)** — `ARCHITECTURE` 582k → **588k** with §8.4's
new **A-95** (~2.2k) and revision 73's in-place corrections inside **A-90 clause 3 / Part 5 residue 4**,
**A-94 Parts 3, 5, 9 and 10** and **A-91** (§8 alone 176k → **180k**; **§0 2.7k → 3.1k** with **position 5
(b)**); §1–§7, §9 and §10 did not move. `ROADMAP` 250k → **255k** with **I-33**, **I-34**, criterion
**rule 9**, sequencing rule **11 (c)**, `I-32`'s corrected exempt figure, `I-30`'s revised item 3 and
dependency line, and the revision-76 ledger entry (Phase 2 alone 179.7k → **183k** as `doc-section` reports
it). **This revision is
CORRECTIONS plus ONE ruling, and NOTHING in it is user-meetable** (sequencing rule 10) — every item is
about the strength of an instrument a reviewer or a builder uses. **A-95** is *the gate that refuses a
write and the report a reviewer reads are not the same instrument*: R69-1 correctly made `--dry-run` take
the same gates in the same order as a real run, which made a **refused** corpus the one change no run can
describe. A-95 separates **class-1 preconditions** (an absence — the run cannot compute an audit; these
still throw everywhere) from **class-2 review gates** (a refusal — the audit is complete and the write is
refused; these may report), and adds **`--review`**, a modifier on `--dry-run` with **no write path**, which
prints the full diff and exits **1**. **A-95 is ~2.2k and it is read ALONE, with A-90 clause 3 and A-94
Part 10 beside it** — a builder or breaker of `I-34` reads those three and nothing else in §8.4. **§0
position 5 gains clause (b)** and **How a criterion is written gains rule 9** — *a stated fault is shown to
fire, or is declared unfireable at its site* — after the fourth instance (A-94 Part 9 fault 3, QA R69-3).
**`I-33` schedules A-91 item 1** (`searchGazetteer` → `{source, hits}`, **94 measured call sites**, 45 of
them in five `.mjs` `qa/` probes that break at runtime) **ahead of the picker, and it owns every re-cut**;
**`I-30` may not land before it**, which is an ordering gate and not a new fence. **Nothing in this
revision gates `I-30`'s design**, which is still fenced by the unresolved visual direction and by nothing
else, and **`CAIRN_VISUAL_ROADMAP` does not move** — this revision opens no `.tsx`, no `apps/web` and no
`docs/design/`. **Three rows moved with round 69's own commits rather than with this revision**:
`QA-FINDINGS`, `BUILD-NOTES` and the corpus — re-measure the first two before quoting the rows below.

**Every row re-measured at revision 72/75 (2026-09-10)** — `ARCHITECTURE` 573k → **582k** with §8.4's
**A-94**, its amendment banners on **A-84 Part 5**, **A-89 Part 5**, **A-90 clause 3** and **A-90 Part 5
residue 2**, and its four in-place corrections inside **A-93 Parts 2, 7 and 9** (§8 alone 168k → **176k**;
**§0 grows 2k → 3k** with **position 12 (d)**); §2, §4, §9 and §10 did not move. `ROADMAP` 245k → **250k**
with **I-32**, the BUILT/SHIP banner on **I-31**, three in-place corrections and the revision-75 ledger
entry (Phase 2 alone 176k → **180k** as `doc-section` reports it). **This revision is ONE ruling and it is
about a deletion two rulings had already made and neither had recorded**: the modal-parent election counted
an **abstention** as a vote, so `CC → null (null:3 AU:2)` and the **entire Cocos (Keeling) Islands**
territory shipped zero rows — `west island`, its capital, returns *no match*. **A-94: an abstention is not
a vote.** It is **READ WITH A-84 Part 5 and A-89 Parts 4 and 5, and NOT with A-93** — A-93's predicate does
not move and its sixteen refusals are asserted unchanged, which is a golden read, not a ruling read. **A
builder or breaker of `I-32` reads A-94 whole, then A-84 Part 5, then A-89 Parts 4 and 5, then A-90 whole,
then A-83 Part 9 clause 1, then BUILD-NOTES KD-127 — and nothing else.** `I-30`'s read is unchanged
**except that A-94 joins the list of rulings it does not read**. **`I-32` is routed builder + breaker,
mandatory** (committed generated data), and **it does not gate `I-30`**, which is still fenced by the
unresolved visual direction and by nothing else. **Three things this revision records rather than rules:**
`I-31` is BUILT and QA round 68 returned **SHIP** on it; the builder was right a third time (**KD-125** —
A-93 Part 2's join names a sovereign for **0** codes as written, **41** on `SOV_A3`), and the ruling is
corrected, not the build; and revision 74's claim that `I-29`'s `apps/web/src/ports/map.ts` edit was
undisclosed **is withdrawn — it was disclosed twice at `59cad74`**, and the fence it broke was
unsatisfiable, which is the architect's defect. **Two rows moved with round 68's own commits rather than
with this revision**: `QA-FINDINGS` and `BUILD-NOTES` — re-measure them before quoting the rows below.
...
(§0 stays 2k); §2, §4, §9 and §10 did not move. `ROADMAP` 236k → **241k** with **I-29**, **I-30**, the
in-place correction to `I-23`'s parent-translation criterion, **Sequencing rule 11** and the revision-73
ledger entry (Phase 2 alone 170k → **174k** as `doc-section` reports it). **The four rulings are
SEPARABLE and only one is about the corpus's contents — do not read them as a family.** A builder of
**`I-29`** reads **A-89 whole, then A-83 Parts 3 and 9, then A-84 Part 5**; adds **A-90 whole with §0
position 11 and A-83 Part 2** for its Part 4; adds **A-92 whole with A-84 Part 7 item 2** for its Part 5;
**and nothing else** — not A-85, not A-86, not A-87, not A-88, not §2 whole, §4 or §10. A builder of
**`I-30`** reads **A-91 whole, then A-82 Parts 4 and 9, then A-83 Parts 6 and 7, then `DESIGN.md` §5 and
§6, and no other §8.4 ruling at all** — including A-89. This revision is a **QA consequence with one thing
beside it that is not**: round 67 confirmed `I-23`'s own subject (**21.5 % → 90.1 %**, reproduced through
the real loader off the committed shards), the builder fixed two of its three MAJORs, and **stopped on the
third and reported** — correctly, because the routed fix deletes nine correct rows to remove one sea
(**KD-122**). The thing beside it is **KD-123**: the pinned GeoNames bytes stopped being served
mid-increment, so **A-90** rules the corpus's supply chain and **§0 gains position 11**. **`I-29` is routed
builder + breaker with a round of its own, and that round also re-cuts the three drifted `qa/` probes named
in ROADMAP revision 73** (`J3`, `B1`, guard `M6` — whose GREEN → RED flip is a strengthening, not a
regression). **`I-30` — the picker — is fenced by the unresolved visual direction and by nothing else;
`I-29` is not a prerequisite for it.** **Three rows moved with QA round 67's and the builder's commits
rather than with this revision** (measured here at `e076253`, because `cairn/CLAUDE.md` is not a file a
breaker or builder edits): `QA-FINDINGS` 402k → **410k**, `BUILD-NOTES` 295k → **311k**, and
`CAIRN_VISUAL_ROADMAP` 72k → **74k** (`.html` **88k**) — **this revision is the first in seven that moves
the visual roadmap**, because `I-22b`, `I-23` and this arc have all landed since its last block and both
files were updated in the same pass. `REVIEW` is unmoved at **52k**.
...
§10 did not move) and `ROADMAP` 224k → **231k** with **I-26**, **I-27**, the BUILT-and-run banner on
**I-25** and the revision-71 ledger entry (Phase 2 alone 162k → **167k** as `doc-section` reports it).
**A-87 is ~8k, it is the entry point to the DERIVE path, and it is READ ALONE — it is not part of the pick
family and needs none of it.** **A builder of `I-26` reads A-87 whole, then A-86 Part 4, then A-59 Parts 2,
3 and 4, and nothing else** — not A-85, not A-84, not A-83, not §2 whole, §4 or §10. `I-22b`'s and `I-23`'s
reads are unchanged. This revision is a **QA consequence** — round 64 confirmed `I-25`'s counter over
sixteen values and both lifecycles and could not break it, and found the level **below** it unguarded: a
`cities` **entry** takes the whole library's travel history down, and in one shape nothing names the row.
Before ruling that, the architect measured the neighbouring fields of the same stored row and found **three
more of the same defect** (`countryCodes` iterable-or-not, `countryCodes` as a string, `attribution`
publishing `NaN`), so **A-87 rules the class rather than the instance** and **`I-26` is routed builder +
breaker, with its round being the one `e1e1973` already owes rather than a second one**. **The standing
sequence is `I-26` → `I-22b` → `I-23` → `I-27`**, and `I-25` is built. **Two rows moved with QA round 64's
commits rather than with this revision**: `QA-FINDINGS` 392k → **396k** and `BUILD-NOTES` 287k → **292k**
(measured here, at `e1e1973`, because `cairn/CLAUDE.md` is not a file a builder or breaker edits). `CAIRN_VISUAL_ROADMAP` is unmoved at **72k** — this revision changes no
rendered surface, opens no `.tsx` and adds no screen, so there is nothing for it to catch up on — and
`REVIEW` is unmoved at **52k**.
...
| `CAIRN_VISUAL_ROADMAP.md` (+ its `.html` twin) | ~74k tok (`.html` ~88k) | never — read the **newest block only**, which supersedes the ones below it (**revision 73's block is the newest**, 2026-09-10: `I-22b`'s map fix, `I-23`'s 7,342 → 149,101 corpus at 21.5 % → 90.1 %, QA round 67's three MAJORs, and revision 70's three rulings — with the built / verified / shippable table, and the plain answer that **nothing but the visual direction fences the picker**) | not a contract doc; skip it for a routine builder/breaker task, and see below for when to *update* it |
...
| **A change to committed generated data** — the gazetteer corpus under `packages/core/src/geo/gazetteer/`, `geo/countries.gen.ts`, or a `fixtures/golden/` file the generator writes | **builder + breaker, mandatory**, and the diff is published in the same commit | ROADMAP sequencing rule 11 / §8.4 **A-90**: the upstream rebuilds daily and archives nothing, so **the committed bytes are the artefact of record and nobody can rebuild them**. A re-pin without its row-level diff is an unreviewed change to the product's core data; a diff over the published cap is a **stop-and-report** |
