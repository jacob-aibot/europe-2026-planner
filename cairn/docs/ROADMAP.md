...
**Revision 59, 2026-09-08.** **No increment added, no increment re-scoped, one bullet corrected — `I-11`'s
*Dependencies / blockers* — and the answer this time is READY.** Revision 54 built that bullet by measuring
the chain against `git log` on `master` instead of against this document's own status cells, and found four
dependencies unbuilt. Revision 55 resolved one by deferring it (`I-10`). The other three have since been
built by builders: **`I-9` at `0e556a0`**, **`I-12a` item 5 at `4b02206`** and **`I-13a` at `9c16984`**. The
bullet had not been updated and still read *"is not built"* for the last two, which is the same class of
staleness the revision-54 check existed to catch — a document asserting a gap that a builder had already
closed. **This revision re-ran revision 54's measurement in full**: every commit hash the bullet cites
checked with `git merge-base --is-ancestor <hash> master`; `I-0` … `I-9`, `I-12`, `I-12a`, `I-13` …
`I-13i` each located in `git log --oneline`; and for the two newest the **code read rather than the commit
message** — `travelStats.ts` computes `obsA`/`obsB` before anything else and tests trigger 3 on them per
§8.4 A-60 Part 6.3, `photos.test.ts` emits its two `A-61` `t.diagnostic` lines *before* their assertions so
a green run publishes 15354 B / 767.7 B per photo and a longest string of 15, and the `< 128` bound carries
its fixture-scoped comment; BUILD-NOTES **KD-81** carries its `RULED … and CLOSED` banner. `npm run
typecheck` exits 0 and `npm run test:tap` reports **1525 tests, 1525 pass, 0 fail**. **`I-9c` is specified
and not yet built and does not block this gate**, which the bullet already stated and now states as a
measurement; `I-10` and `I-13f` remain deferred with their triggers, and the gate **certifies** those rather
than waiting for them. **Nothing else moves**: no phase, step, criterion, dependency or version, no `.tsx`,
no `qa/`, no `cairn/docs/design/`, no `ARCHITECTURE.md` — this is a status correction, not a design change.
...
**Revision 67, 2026-09-09.** **QA round 60 measured `I-21` against 171 real destinations and it answers
21.5 % of them. That is a verdict on the capability, not a trigger to wait on.** Control set (50 large
cities): **100 %**. Travel set (72 famous villages, 23 islands, 26 park gateways): **26/121 = 21.5 %** —
`hallstatt`, `positano`, `zermatt`, `sintra` and `geneva` return `no match`, and `obidos` returns Óbidos
**Brazil**. **The cause is a design mistake with my name on it** (**R60-1**, MAJOR):
`ne_10m_populated_places` is a cartographic layer that selects by administrative rank and `POP_MAX`, and
travel destinations are selected by **notability** — Hallstatt has 779 residents, roughly a million
visitors a year, and alternate names in **27 languages**. A-82 Part 11 item 3 deferred the bigger dataset
behind *"misses observed against real use"*; the trigger has fired with a number.
`ARCHITECTURE.md` revision 64's §8.4 **A-83** rules it; **`I-22` is rewritten and `I-23` is new.**

- **The curve was measured, not a threshold picked blind.** A-83 Part 1 publishes eleven points from
  `l>=12` (71.9 % travel, 3.04 MB on disk) to every Wikipedia-linked feature (95.0 %, 20.89 MB), each with
  its **bytes fetched per search** separated from its **bytes on disk**, so the choice is Jacob's with real
  figures in front of him. The point A-83 adopts is **`languages ≥ 4` or `wikipedia and population ≥
  1,000`** — 149,097 rows, **90.1 %** travel, control still 50/50.
- **Sharding is what changes which number matters.** The corpus ships as **JSON documents behind the
  dynamic import that already exists**, split adaptively while any exceeds 96 KiB, one shard per token
  prefix; **a search fetches exactly one — ~20 kB gzipped, 44 kB worst case — against today's 197 kB
  chunk fetched in full the first time anyone types a letter.** Coverage goes up more than fourfold and the
  bytes a session fetches go **down**. That is the answer to Jacob's *"2 is two heavy"*: there is no ~1 MB
  payload in this design.
- **The 98 refusals and the `City.centre` migration are ONE increment, as Jacob directed** (**R60-2**,
  MAJOR). Ranked by population the refused set opens **Brazzaville, Geneva and Jerusalem**, not the four
  small border towns A-82 Part 5 published. The invariant is **restated, not weakened** — *no shipped row
  may **silently** contradict the index* — so disagreeing rows ship carrying the disagreement, published by
  name; and that needs **`City.placeId`**, the gazetteer row a human **picked**, which is the exact
  provenance A-29 Part 3 item 3 said was missing. **`City.centre` becomes `LatLng | null`** in the same
  increment. `SCHEMA_VERSION` **3 → 4**, `SUMMARY_VERSION` **5 → 6**.
- **`I-22` before `I-23`, and both are independently shippable and independently useful.** `I-22` migrates
  the record, amends A-29's precedence by one clause and **regenerates the CURRENT dataset** so the 98
  disagreements ship — Geneva, Jerusalem and Brazzaville become findable **without waiting for the
  swap**. `I-23` then swaps the dataset and the representation and changes no record shape at all, which
  is what makes it attackable on its own.
- **Round 60's seven MINORs are folded into the two increments rather than queued.** The `İ` pair pins
  nothing about the fold's ordering (**R60-3** — all 24 permutations measured, six green, and KD-113's
  premise that U+0130 has no canonical decomposition is **false**); no shipped row may render as a bare
  name or carry an unreadable one (**R60-4**, **R60-9**); labels are distinct within a result set
  (**R60-9**, measured at 23 of 171 corpus queries today); the round-trip check compares structures rather
  than two strings (**R60-5**); `limit` is validated (**R60-6**); A-82 Part 3's printed type is corrected
  (**R60-7**, documentation, done in revision 64); the golden-scan exemption narrows to one name
  (**R60-8**).
- **Both increments are routed builder + breaker, MANDATORY.** `I-22` is a schema migration touching
  `SCHEMA_VERSION`, `SUMMARY_VERSION` and A-29's precedence; `I-23` is a dataset swap, a representation
  change and a new licence obligation. Either alone earns the round.
- **Scope fence, unchanged and restated because a builder will otherwise drift into it: NO `.tsx`, no
...
**Revision 72, 2026-09-09.** **QA round 65 ran over `e1e1973` + `I-26` as one subject and sent it back with
two MAJORs, both of them the architect's.** `ARCHITECTURE.md` revision 69's §8.4 **A-88** rules them and
**`I-28` builds it**. Round 65 confirmed the increment's own subject *works* — the five shapes return
instead of throwing, the row and path are named on the **success** path, absorption is lifecycle-blind over
sixteen values and both lifecycles, eleven mutants are RED, the channel is privacy-clean, `absorbed` is
deterministic, and no healthy trip can produce a false-positive absorption. What it broke is beside it.

- **R65-1 is a live regression and it is the only thing in this round that gates anything.** A-87 Part 6
  widened `rowStatsReadable` **in place** and never named its one production caller — `travelHistory`'s
  `catch`, where A-59 Part 4 names **exactly one** suspect row. Widening the predicate widened the suspect
  set over precisely the rows `I-26` had just made **non-fatal**, so `rowId` fell from `"the-broken-one"` to
  `null` and `WorldMap.tsx`'s named-row arm went dark: the banner says *"one of these two"* for a failure
  one row caused. **The failure branch now carries one population — `rowDatesReadable`, the rows that can
  still produce the throw** — and A-59 Part 4's *"exactly one"* rule stands verbatim over it. The **wide**
  fact stays where A-87 Part 4 put it: `TravelStats.absorbed`, on the success path, with the row **and** the
  path. Two lines of `packages/client`, no `.tsx`.
- **R65-2 is the coverage claim, falsified at its own stated condition (b).** The reader descends into
  **four** stored record classes; the compiler-maintained denominators cover **two**. A third
  `AttributionCensus` number compiles green, gated by nothing, reported by nothing and expected by no cell.
  **Two axes are added** — `Record<keyof TripSummaryRow['attribution'], true>` and
  `Record<keyof AttributionCensus, true>`, both test-local — **and the obligation is stated generally**: a
  coverage claim denominates every record class its reader descends into, each by a constant the compiler
  maintains, and the ruling that adds a fifth class adds its axis in the same increment. Where that is not
  affordable, the sentence is narrowed instead. A-87 Part 7's qualifier — *"which today means a record class
  added after this ruling"* — was false when it was written, and it is withdrawn.
- **Four riders, each one file and each measured.** An **absent** census number was reported as a defect and
  the readable number beside it discarded, which inverts A-87 Part 2's own arm 1 — `{located: 5}` now
  publishes **5** (**R65-3**). `isPlainObject` admits `Date`, `Map`, `Set` and `Error`, so
  `cities: [new Error()]` puts a city named **"Error"** on the lifetime map absorbing **nothing** and
  `attribution: new Date()` drops both censuses in silence — **A-87 Part 7's *"plain data"* premise is
  false** and is corrected; its **accessor** premise is right and was verified (**R65-5**). Two residues
  were written in directions that could not fire: rule 5's trigger argued the opposite of what it recorded,
  and residue 3's memo remedy saves nothing (measured: 0.99 ms per row against 0.71 ms for the whole
  library, and R65-1's fix removes the only per-row caller anyway) — both corrected in place (**R65-7**,
  **R65-8**). **R65-6** is A-87 Part 10 residue 1's own cap trigger firing **inside** the increment that
  added the surface; it is discharged, not deferred a second time.
- **One thing the architect found while ruling, recorded as such.** `stopCount`, `poolCount` and
  `placeCount` read a stored `'x'` as `0` and report **nothing** — a third convention for a stored number,
  beside the census numbers, which absorb. They are the reader's last exception and they go with the rest.
- **§0 gains position 10, and it is the general form of both MAJORs**: *a claim is only as strong as the
  mechanism that maintains it.* (a) a coverage claim's denominators are compiler-maintained, all of them;
  (b) a predicate with more than one consumer is not widened in place until the ruling has named every
  consumer; (c) where neither is affordable the sentence is narrowed — and a later finding that a claim is
  broader than its mechanism is a **defect in the claim**, a MINOR to the architect, which does **not** gate
  a product increment. **Sequencing rule 10** is the routing half.
- **Ordering: `I-28` first, then `I-22b`, then `I-23`, then `I-27` — and `I-28` is the LAST increment of
  this arc.** `I-28` is queued ahead for one reason and it is not that a finding exists: **R65-1 is a
  degradation a user can see today, introduced by this arc**, and it costs one expression. Everything else
  in `I-28` rides along because it is in the same three files and costs one test run, and stop-and-report
  condition 4 drops any of it that turns out to cost more, re-queued **behind** `I-23`. `I-22b` shares no
  file with it. `I-27` stays behind the picker work.
- **What is NOT queued, said plainly, because five rounds of this arc have each produced work.** **R65-2 and
  R65-5 buy a user nothing today** — no shipped write path produces a partial census, a `Date` in
  `cities[]` or a fifth record class, and both are reachable only through storage a user hand-edited. They
  are in `I-28` because they are cheap and in the same files, **not** because they are urgent; if they were
  expensive they would be narrowed sentences instead, which A-88 Part 11 says in as many words. **After `I-28`
  and the one round it owes, the read-gate class gets no further mandatory adversarial round.** A seventh hostile
  shape of a covered field is a **builder** finding against the table's shape set (A-39 Part 11's line,
  A-87 Part 7's own (c)) — not a ruling and not a round.
- **The product line, which is what actually decides this queue.** Jacob's goal is unchanged and it is not
  this arc: past-trip entry, someone typing where they have been and getting a real country out of it.
  **`I-23` — the notability corpus, ~90% coverage against today's roughly one in five — is the increment
  that delivers it, and it has been queued behind this arc for five rounds.** Only `I-28` Part 1 blocks it.
- **Routing, and the pipeline state stated plainly.** **`ede933f` is built and was adversarially verified;
  the verdict was SEND BACK. Nothing in this arc has a manager verdict of SHIP.** `I-28` is **builder +
  breaker** — a core invariant and a client selector's population — and **its round is a new one**, because
  unlike rounds 64 and 65 there is no owed round to fold it into. **That round does not gate `I-23`:**
  `I-28` shares no file with `I-23`, so `I-23` is built and reviewed without waiting on it, and anything
  `I-28`'s round finds routes as a normal finding rather than as a stop on the product path.
...
- ~~**An undisclosed out-of-fence edit is recorded rather than reverted, and it is the third in this
  arc.**~~ **⚠ CORRECTED AT REVISION 75 (QA R68-7, ARCHITECTURE §8.4 A-94 Part 7 item 7). THE EDIT WAS
  DISCLOSED, AND THE ERROR WAS MINE.** `I-29` changed **`apps/web/src/ports/map.ts`** — `MapBoundsLike` →
  `MapBounds` at five sites plus the import. It is mechanical, correct and the unavoidable consequence of
  A-92's deletion; **it stands**, and so does the **out-of-fence** half. What this bullet said — that it was
  *"not disclosed in the builder's report"*, a third instance after **R62-7** and **R64-7** — **is false**:
  `BUILD-NOTES.md` at `59cad74` discloses it **twice**, in the addendum header (*"zero `apps/web/` except
  A-92's one type-name change that ROADMAP `I-29` carves out"*) and in **PART 5**, which names the file and
  the change. **R62-7 and R64-7 were real; this was not, and the count is two.** The remaining fault is
  mine: `I-29`'s fence — *"no `apps/web` file of any kind"* — was **unsatisfiable** alongside A-92's own
  ruling and the exit criterion `npm run typecheck` exit 0. **A builder who cannot satisfy two of my
  sentences at once, picks the ruling over the fence and says so, has done the right thing**; the defect is
  in how the fence was written, and a fence that forbids what a ruling in the same increment requires is a
  design defect routed to me. A builder's report still names every file it changed — that half of the rule
  is unchanged, and this builder met it.
...
**Revision 75, 2026-09-10.** **`I-31` is BUILT and QA round 68 returned SHIP on it — and the same round
found that an entire inhabited territory had already been deleted from the picker, by neither ruling and
recorded nowhere.** `ARCHITECTURE.md` revision 72's §8.4 **A-94** rules it; **`I-32` builds it**; three
sentences in this file are corrected in place.

- **`I-32` is a user-meetable defect, and it is the only one round 68 found** (sequencing rule 10).
  `west island` — the capital of the **Cocos (Keeling) Islands** — returns **no match**, `cocos` returns a
  town in Brazil, and the territory ships **zero rows**, because the modal-parent election counts an
  **abstention** as a vote: `CC → null (null:3 AU:2)`, every row ships with no country and no region, and
  clause 1 refuses all five as bare names. **A-94: an abstention is not a vote**, the per-code tally is
  published in a golden, and the five rows come back as `AU` (corpus **149,085 → 149,090**). **Its price is
  declared, not discovered** — `TK → NZ`, so the **shipped `null` arm goes to zero**, which A-89 Part 5
  sentence 3 required be said out loud, and it is said in both documents.
- **The other two MAJORs are provenance, and neither reaches a user.** **R68-2/KD-125**: A-93 Part 2's
  `P(c)` join names a sovereign for **0** codes as written and **41** on `SOV_A3` — the ruling's own figure
  — so built literally it ships its own fault 4 and deletes Jersey, Guernsey, Alderney, the Faroes,
  Signilskär and Norfolk Island. **The builder built the measurement, disclosed it, declined to edit the
  ruling, and is upheld**; the sentence is corrected in `ARCHITECTURE.md`. **R68-3**: an **ordinary**
  regeneration re-seeds the append-only source log, and the chain test is green for a tail truncation, green
  for a heads-only rewrite and vacuous at 5 entries — so A-90 clause 3 is amended and `I-32` carries the
  fix: the log is written only under `--repin`, never re-seeded, and its witness is a **prefix literal in a
  test file the generator cannot write**.
- **Three corrections in place, all mine.** `I-31`'s *"no refused row of **any** reason has feature class
  `P`"* → scoped to `'multi-country'` (**R68-6**, KD-126 — **the builder was right**, `Bantam Village` is a
  `P/PPL` `'bare-name'` refusal that predates the increment). `I-31`'s N1 arithmetic **16 → 169** → **16 →
  102** (**R68-5**; the builder's disclosed doubt was correct). And revision 74's undisclosed-edit bullet
  (**R68-7**): `I-29`'s `apps/web/src/ports/map.ts` change **was disclosed**, twice, at `59cad74` — it is
  **not** a third instance beside R62-7 and R64-7, the out-of-fence half stands, and **the fence I wrote was
  unsatisfiable alongside A-92 plus a clean typecheck**, which is my defect.
- **Ordering is unchanged: `I-32`, then `I-30`, and `I-32` does not gate `I-30`.** `I-32` is `tools/`,
  `packages/core/test`, four goldens and the corpus; it touches no `.tsx` and no door the picker calls. If
  it lands first the picker inherits five rows it would otherwise be missing; if it does not, `I-30` ships
  with a published five-row hole and says so. **The picker is still fenced by one thing and it is the
  unresolved visual direction.**
- **Sequencing rule 12 gains (d)** — *a rule that aggregates evidence says what a silent witness contributes
  and publishes the tally* — and **KD-127's remedy is in `I-32`**: the four audit numbers move into the
...
> **BUILT (`58858cb`) and through its adversarial round — QA round 60, SEND BACK, 0 blockers, 2 MAJOR.
> Revision 67 banner: read this entry as history, and read `I-22` and `I-23` for what is being built now.**
> Round 60 measured this increment against 171 real destinations: **100 %** of the 50-city control and
> **21.5 %** of the 121-destination travel set. The mechanism survived everything — the invariant
> re-derived to the row, determinism byte-identical over three runs, 180 ranking permutations with zero
> reorderings, the bundle boundary intact. **What is wrong is what the mechanism can reach**, and it is a
> defect in the ruling rather than in the build: the filter was on population, and travel destinations are
> selected by notability. `ARCHITECTURE.md` revision 64's **A-83** is the answer, **`I-22`** carries the
> migration and the disagreeing rows, and **`I-23`** carries the dataset and the representation. **Nothing
> in this entry was built wrongly**; three of its verification criteria are corrected by `I-23` and named
> there — the old **N3** is deleted as a fault that fails nothing, the label criterion's *"contains a
> country name"* is made literally true by a shipping refusal, and the `[snapshot]` refusals golden becomes
> the disagreements golden.
...
and this increment does not touch the dataset's selection, representation or sharding. Do **not** read §2
whole, §4, §10 or any of A-76…A-81.

**Why it exists, and it is now two reasons rather than one.** `createTrip` writes `centre: {lat:0, lng:0}`
for a city nobody located — *"a value nobody measured wearing the shape of one"*, which is **not** A-26
Part 1's honest `null` — and A-56 widened `TripSummaryCity` with a non-nullable `centre` on the stated
ground that `City.centre` is non-nullable, so **every hand-entered city today is a summary row claiming to
be at 0°N 0°E and the first surface that draws city pins will draw them there.** And QA round 60 measured
that the 98 rows A-82's consistency invariant refuses open **Brazzaville, Geneva and Jerusalem** — 19 of
them above 100,000 people and two of them national capitals — so `node cli.ts cities geneva` answers
`no match` for the second city of Switzerland. **The two are one increment because the fix for the second
is a field on `City`, and adding two fields in one migration is one adversarial round instead of two.**

**What it is NOT.** It is not a form, not a screen and not a UI decision — **no `.tsx` is opened, no file
under `apps/web`**. It is not the dataset swap: the selection rule, the sharding and the JSON
representation are **`I-23`**, and a builder who finds themselves editing `tools/gen-gazetteer.mjs`'s
*source* or its *shard layout* has left the increment. The generator is touched here for exactly one
reason — to stop refusing the disagreeing rows and start marking them.

- **Built, in four parts, in this order. Part 1 is the record and nothing downstream moves until it
  typechecks.**
  1. **The two fields, in `packages/core/src/model/types.ts`.** `City.centre: LatLng | null` and
     `City.placeId: string | null`; `CityInit` gains both, both optional, both defaulting to `null`.
     `SCHEMA_VERSION` **3 → 4** with its own line in the constant's docstring. `placeId` is documented as
     **what a human picked, never what a system matched** — A-82 Part 6's fence is what makes the
     precedence in part 3 safe, and the docstring says so where the next builder will read it.
  2. **The serialization and the migration.** `fromJSON` accepts `centre: null` and a `placeId` string or
     `null` and refuses anything else with a named reason; `toJSON` emits both. **`migrate.ts` gains a
     3 → 4 rung**, and its behaviour is a ruling, not a convenience: **a stored `centre` exactly equal to
     `{lat: 0, lng: 0}` becomes `null`; every other value passes through untouched; `placeId` is filled
     with `null`.** The rung **counts** what it converted and the count is reported, on `migrate.ts`'s
     existing model. Stated plainly because it will be questioned: this also nulls a city genuinely at
     0°N 0°E, of which there are none on land, and the alternative is carrying a fabrication forever.
  3. **The precedence, in `derive/summary.ts`, and it is A-29's first amendment.** `TripSummaryCity.centre`
     becomes `LatLng | null`; `countrySource` gains `'picked'`; the per-city rule becomes, in order:
     `centre === null` ⇒ no coordinate attribution; `placeId !== null` ⇒ **the city's own `countryCode`
     wins, `countrySource: 'picked'`**; otherwise **A-29 Part 3 verbatim and unamended** — `countryOf`
     first, the four-step gate only in its silence. `SUMMARY_VERSION` **5 → 6**.
     `derive/travelStats.ts` counts a null centre as **unlocated** in `located` / `unattributed.cities`
     and `TravelStatsCity.centre` becomes nullable; `derive/geoCheck.ts`'s `centres` map is built from the
     cities that **have** a centre, so a city with none contributes no anchor (it contributes a useless
     one today, which is harmless and stops being possible).
  4. **The generator stops refusing, and starts marking.** `GazetteerRow` gains
     `indexAgrees: boolean`; `tools/gen-gazetteer.mjs` emits a contradicting row with `indexAgrees: false`
     instead of dropping it; `fixtures/golden/gazetteer-refusals.json` is **renamed
     `gazetteer-disagreements.json`** and changes job — it publishes every such row with **both** answers,
     `{id, name, statedCountry, derivedCountry}`, and those rows now **ship**. `GazetteerRow.id` gains its
     source prefix (`'ne:…'` today, `'gn:…'` after `I-23`) so that a persisted `placeId` says which dataset
     minted it. `cli.ts cities` marks a disagreeing hit in its output line, which is how a tester sees the
     whole of this increment with no browser and no UI.
- **Not built, and named so nobody adds it.** **No `.tsx`, no `apps/web` file of any kind.** **No change
  to the gazetteer's source, its selection rule, its representation, its sharding or its byte budget** —
  all `I-23`. **No new `IssueCode`** and no `validateTrip` rule for `placeId`: A-74 Part 4 decides it —
  the parser already refuses a non-string, and a cast is not a producer. **No `homeBase` change** (KD-55,
  a fifth time). **No auto-matching of a typed name to a row** — A-82 Part 6 forbids it and this
  increment's precedence rule is exactly why it must stay forbidden. **No `qa/` file, no
  `docs/design/`.**
...
- **Architecture / data model.** `City` gains two fields; `SCHEMA_VERSION` **3 → 4**; `SUMMARY_VERSION`
  **5 → 6**; `TripSummaryCity.centre` and `TravelStatsCity.centre` become nullable; `countrySource` gains a
  ...
  move and stays at 87.** No port, no selector, no conflict rule and no screen.
- **Verification.** Tagged per **How a criterion is written**. Every injected fault is run
  **red-before-green** and its measured output recorded; a criterion asserted rather than run is not
  discharged.
  - **The precedence, and it is the criterion this increment lives or dies on** `[stated]`: over a
    hand-built `Trip`, a city with `placeId: 'ne:x'`, `centre` inside France's ring and
    `countryCode: 'CH'` reports **`{countryCode: 'CH', countrySource: 'picked'}`**; the *same* city with
    `placeId: null` reports **`{countryCode: 'FR', countrySource: 'coordinate'}`**. **N1, injected:** drop
    the `placeId !== null` test from the gate → the first assertion fails naming `CH`. **N2, injected:**
    make the picked arm fire when `placeId` is `null` → the second fails naming `FR`. **N2 is the fault
    that would let a mistyped `HU` on Vienna put Hungary on a lifetime map permanently**, which is A-29
    Part 3 item 3's own objection, and it must be run.
  - **`null` is a first-class centre, as a ceiling** (rule 4) `[stated]`: over a trip with one located and
    one unlocated city, `tripSummary().cities` has **exactly two** rows, **exactly one** with
    `centre === null`, and `travelStats` counts **exactly one** unlocated city and **zero** cities at
    `{0,0}`. **N3, injected:** restore `createTrip`'s `{lat:0, lng:0}` default → the zero-at-origin
    assertion fails naming the city.
  - **The migration** `[stated]`: a schemaVersion-3 fixture carrying one city at `{0,0}` and one at a real
    coordinate migrates to **exactly one** `null` centre and **exactly one** unchanged coordinate, with
    `placeId` `null` on both, and the rung's reported conversion count is **1**. **N4, injected:** make the
    rung null every centre → the unchanged-coordinate assertion fails naming the city. **N5, injected:**
    omit the rung → `migrateDoc` throws its own "no rung" error rather than silently accepting, which is
    the ladder's existing guarantee and must be shown to still hold.
  - **The disagreeing rows ship, as a ceiling** `[stated]`: `searchGazetteer('geneva', …)` returns a row
    with `countryCode: 'CH'` and `indexAgrees: false`; the same for `jerusalem` and `brazzaville`. **Over
    every shipped row with `indexAgrees: true`, `countryOf(row.centre, COUNTRY_INDEX)` is either the row's
    own code or `null` — zero exceptions, no allowlist.** That is round 60's own test and it must be
    carried across unchanged. **N6, injected:** set `indexAgrees: true` on the emitted disagreeing rows →
    the zero-exception test fails naming Geneva or another of the set. **A disagreement count of zero is
    itself a failure**: the marking that never fires is the marking that was deleted.
  - **The golden** `[snapshot]`, paired as rule 2 requires with the `[stated]` zero-exception test above:
    `gazetteer-disagreements.json` is written by the generator, carries `$generatedBy`, `$source`,
    `$sourceSha256` and `$what`, and names every row with **both** country answers.
  - **Negative controls, which must stay green** — a red here is a defect in this increment: the reference
    trip's summary and `travel-stats.json` change **only** in the fields this increment moves, and the
    diff is recorded field by field; `countries.json`, `country-holes.json` and `forgiveness-drops.json`
    are **unchanged**; A-29's four-step gate has **no new caller and no changed clause** for a city with
    `placeId: null`, asserted by the second half of the precedence criterion.
  - **Regression** `[stated]`: `npm run test:tap` green with the new tests added. **Every test that moves
    ...
    other test that needs editing is a finding, not an edit.
- **Dependencies / blockers.** **`I-21`, built and on `master`, and through its adversarial round (QA
  round 60).** Nothing else. **`I-23` waits on this** — the disagreeing rows cannot ship until `placeId`
  exists to make them safe.
- **Ship gate.** `npm test` green; `npm run typecheck` exit 0 on **both** projects; `npm run web:build`
  succeeds and the main-chunk figure is recorded against A-82 Part 9's **2 kB** ceiling; the export count
  re-measured and **87**; **N1–N6 each run red-before-green with their measured output recorded**; the
  generator run **twice** byte-identical and `--audit-only` recorded; the migration exercised on a real
  schemaVersion-3 document and the conversion count recorded. **Files touched:
  ...
  `derive/geoCheck.ts`, `geo/gazetteer.ts`, `geo/gazetteer.gen.ts`, `tools/gen-gazetteer.mjs`, `cli.ts`,
  `fixtures/golden/gazetteer-disagreements.json` (renamed), the fixtures and tests the two version bumps
  reach, and `docs/BUILD-NOTES.md`. Nothing else** — no `.tsx`, no `apps/web`, no `packages/client/src`,
  no `qa/`, no `docs/design/`, no lockfile, no new dependency.
- **Route: builder + breaker, MANDATORY**, then the manager. `cairn/CLAUDE.md`'s delegation table: a
  schema migration **and** an amendment to a shipped precedence rule, either of which alone earns the
...
> **⚠ BUILT (`ede933f`) and adversarially run at revision 72 — SEND BACK, 0 BLOCKERS, 2 MAJOR, 9 MINOR.**
> QA round 65 covered `e1e1973` + `I-26` as one subject, as this entry says it must. **Both MAJORs are the
> architect's, not the builder's** — the builder implemented A-87 as ruled. **R65-1 is a regression this
> increment introduced**: Part 6's in-place widening of `rowStatsReadable` broke A-59 Part 4's named-row
> attribution, which is a sentence a user reads. **R65-2** falsifies A-87 Part 7's coverage claim at its own
> stated condition (b). §8.4 **A-88** rules both plus four riders, and **`I-28` is the repair pass** — read
> this entry as history and build `I-28`. What round 65 confirmed **fixed** stays fixed: the five shapes
> return instead of throwing, each names its row and path on the success path, absorption is
> lifecycle-blind, and the door reads `CityInit.centre` exactly once.
...
#### I-28 — the named row comes back, the census keeps the number it could read, and the covering claim gets its other two denominators (revision 72, `ARCHITECTURE.md` revision 69's §8.4 **A-88**, QA **R65-1**/**R65-2** MAJOR; **R65-3**, **R65-4**, **R65-5**, **R65-6**, **R65-7**, **R65-8**, **R65-9** ride along)

**Read §8.4 A-88 whole — it is ~6k — then A-87 Parts 2, 3, 6 and 7, then A-59 Part 4. Nothing else.** Do
**not** read A-86, A-85, A-84, A-83, §2 whole, §4 or §10. **Do not reopen** A-87 Part 2's three-way rule,
Part 3 rules 1, 2 and 3, Part 5's lifecycle-blindness, or anything at a build door. This increment is a
**repair pass on the read side**, and it is the last increment of this arc.

**Why it exists.** `I-26` shipped a regression in the one thing the read-gate arc exists to protect: a
sentence on a screen that names the row that broke. A-87 Part 6 widened `rowStatsReadable` **in place** and
did not name its one production caller — `travelHistory`'s `catch`, `packages/client/src/selectors/index.ts:291`,
where A-59 Part 4 names **exactly one** suspect row and stays `null` for two or more. Measured over a library
of one shape-invalid `startDate` (still throws, A-37 Part 2) plus one row whose only fault is absorbed:
`rowId` was `"the-broken-one"` at `e1e1973` and is **`null`** at `ede933f`, so `WorldMap.tsx:100` prints
*"One of the stored trip records is not readable"* for a failure one row caused. Beside it, five smaller
things the same ruling left: an **absent** census number is reported as a defect and the readable number
beside it is thrown away; `isPlainObject` admits `Date`, `Map` and `Error`, so `cities: [new Error()]` puts
a city named **"Error"** on the lifetime map absorbing nothing; the covering table denominates two of the
**four** record classes the reader descends into; the three `countOf` counts are the reader's last silent
field; and `cli.ts` prints a 3,529-character line for a row with 200 absorptions, which is A-87 Part 10
residue 1's own trigger firing inside the increment that added the surface.

**What it is NOT.** **No `.tsx`, no `apps/web` file of any kind** — the fix to R65-1 is measured at
`travelHistory`'s return value, and `WorldMap.tsx` is unchanged because it already reads `rowId`. **No
rename** of `rowStatsReadable`, `unreadableRows` or `rowDatesReadable`. **No narrowing of
`rowStatsReadable`** — it keeps the wider subject A-87 Part 6 gave it and simply stops being asked the
failure branch's question. **No new store method and no recompute affordance** — still `I-27`. **No cap on
any stored count** (A-86 Part 5 stands). **No new runtime export, no new type export, no new `IssueCode`, no
corpus change, no migration rung.** **No version movement**: `SCHEMA_VERSION` **5**, `SUMMARY_VERSION` **8**,
`ROW_KEYS` **15**, A-39 Part 11's covering table **45**, §2.10's **runtime** export count **88**.

- **Built, in five parts, in this order. Part 1 is the only one a user can see today.**
  1. **`travelHistory`'s suspect population becomes `rowDatesReadable`** (A-88 Part 2). One expression in
     the `catch` and the `unreadableRows` docstring, which states the population in a clause and points at
     A-88 Part 2 for why. `rowStatsReadable`'s docstring gains one sentence: it has no production caller
     until A-59 Part 5's surface exists, and it is an identity with core so it cannot drift while it waits.
  2. **`readCensus` gates its two numbers independently** (A-88 Part 5). Absent or `null` → `0`, uncounted,
     **no absorption**; a count → the count; anything else present → `0` **and** one `census` absorption at
     `attribution.places.located` / `.attributed`. The container arm is unchanged. The docstring's fourth
     arm — *"a census that does not carry both of its declared numbers is not a census"* — comes out.
  3. **One stored-record predicate at all three object gates** (A-88 Part 6): not `null`, not an array, and
     the prototype is `Object.prototype` or `null`. A present value that is not a stored record is a defect
     at **its own** level — one `entry` absorption at `cities[i]`, one `census` absorption at `attribution`
     or `attribution.places` — and the level below is not asked. **The three `countOf` fields move into the
     reader** and absorb at `stopCount` / `poolCount` / `placeCount`, kind `'field'`, with the value they
     publish unchanged at `0`.
  4. **The covering table gains its other two axes** (A-88 Part 4), test-local:
     `Record<keyof TripSummaryRow['attribution'], true>` and `Record<keyof AttributionCensus, true>`, each
     key crossed with the hostile shape set, each cell carrying one stated expectation. The shape set gains
     a **non-stored-record** shape (a `Date`) on every axis. Cells that move by ruling and not by accident,
     listed so a mover is visible: `attribution: {places:{}}` *absorbed* → **inert**; `placeCount` /
     `stopCount` / `poolCount` present-and-wrong *inert* → **absorbed**; the new `Date` cells. **R65-9's
     one line rides here**: `Outcome` states the `kind`, so the table stops reading it off the value under
     test.
  5. **The three small ones.** R65-6: `cli.ts` prints at most **five** paths per row, then *"…and N more"*.
     R65-4: bind `cities[i].firstDay` and `.lastDay` once each — R64-1's own fix, one layer in. Both
     residues A-88 corrects (A-87 Part 3 rule 5's trigger and Part 10 residue 3's remedy) are **document
     work already done in A-88**; the code comments quoting them are updated to match, which is a comment
     diff and nothing else.
- **Verification.** Tagged per **How a criterion is written**, including rule 8.
  - **N1** `[measured, red-before-green]`: the two-row library of A-88 Part 1 — `the-broken-one` with
    `startDate: 'not-a-date'` and `merely-absorbed` with `cities[0].countryCode: 'at'` — returns
    `ok: false`, `rowId: "the-broken-one"`, `unreadableRows: ["the-broken-one"]`. **Injected:** point the
    filter back at `rowStatsReadable` → `rowId: null` and this reddens. **Second arm, the other side:**
    a library of **two** shape-invalid rows returns `rowId: null` with **both** ids in `unreadableRows`, so
    the *"exactly one"* rule is pinned in both directions (A-34 Part 4).
  - **N2** `[measured]`: `attribution: {places: {located: 5}}` publishes `located.places` **5**,
    `unattributed.places` **5**, and **zero** absorptions; `{places: {}}` and `{places: null}` publish
    `located.places` **0** with **zero** absorptions; `{places: {located: '5', attributed: 2}}` publishes
    `located.places` **0**, `attributed` **2** and **one** absorption at `attribution.places.located`.
  - **N3** `[measured]`: `cities: [new Error('boom')]` produces **one** `entry` absorption at `cities[0]`,
    **no** city named `"Error"`, `seen.cities` **1** (A-87 Part 3 rule 1, unchanged) and `unnamedCities`
    **0**; `cities: [new Date()]`, `[new Map()]` and `[[]]` do the same; `attribution: new Date()` produces
    **one** `census` absorption at `attribution` and both censuses contribute nothing.
  - **N4** `[measured]`: `stopCount: 'x'` publishes the same numbers it does today **and** one `field`
    absorption at `stopCount`; a healthy row absorbs nothing, over the whole reference library.
  - **N5** `[injected, type-level]`: adding a third key to `AttributionCensus` and a third key to
    `TripSummaryRow['attribution']` each fails `tsc` **inside the covering table** — the control being A-88
    Part 4's own repro, in which today it fails only at fixture literals and `readCensus`' return.
  - **N6** `[measured]`: a row with 200 unreadable city names prints one `cairn stats` line of **under 200
    characters** ending in *"…and N more"*, and the presence arm of A-87 Part 8 item 4 still holds (the
    block prints for a library that carries an absorption).
  - **N7** `[measured]`: counting accessors report **1** read for every gated field of a `cities` entry,
    `firstDay` and `lastDay` included — R65-4's `{"firstDay":2,"lastDay":2}` becomes `1` and `1`.
  - **Non-regression** `[measured]`: over the committed reference library, `travelStats` reports
    `absorbed` **[]**, `seen.places` **95** against `located.places` **94**,
    `unattributed ≤ located ≤ seen` for all three classes, `unnamedCities` **0**, `unreadableCityDates`
    **0**, `unreadableCityLists` **0**, and `travelHistory` is `ok: true` — every figure unchanged from
    `I-26`. `qa/r65-gate.mjs` §L1/§L2 and §C go **green**; `qa/r64-census.mjs` stays **0 FAIL / 69 ok**;
    `packages/core/test/cityPick.test.ts`, `pickCentre.test.ts` and `nullCentre.test.ts` are **green
    unedited** — this increment touches no door.
  - **Regression** `[stated]`: `npm run test:tap` green. **No golden moves** — a healthy row absorbs
    nothing, so `travel-stats.json` is byte-identical; a golden needing regeneration here is a **finding,
    not an edit**.
- **Dependencies / blockers.** `I-26`, built, on `master` at `ede933f`. Nothing else. It touches
  `packages/core/src/derive/travelStats.ts`, `packages/client/src/selectors/index.ts` and `cli.ts` — the
  same three files `I-26` touched — and shares **no** file with `I-22b` or `I-23`.
- **Ship gate.** `npm test` green; `npm run typecheck` exit 0 on **both** projects;
  `Object.keys(core).length` re-measured and **88**; **N1–N7 each run red-before-green with their measured
  output recorded**. `npm run web:build` is **not** owed — no `apps/web` file and no `packages/client`
  export moves — and `qa/i7a-idb-rowkeys.mjs` is **not** owed, because `ROW_KEYS` does not move; say so
  rather than leaving either absence to be read as an omission. **Files touched:
  `packages/core/src/derive/travelStats.ts`, `packages/client/src/selectors/index.ts`, `cli.ts`,
  `test/stats-storage.test.ts`, `test/cli.test.ts`, `packages/core/test/` and `packages/client/test/` as the
  assertions require, and `docs/BUILD-NOTES.md`. Nothing else.**
- **Route: builder + breaker.** It changes a core invariant and a client selector's population.
- **Four stop-and-report conditions, and the fourth is the scope fence.** (1) Any golden moving. (2)
  `Object.keys(core).length` landing anywhere but 88. (3) Any cell of the covering table whose measured
  outcome is neither *absorbed*, *inert* nor *throws* — that is a shape-set finding and it is reported, not
  ruled around. (4) **Any part of this increment other than Part 1 that cannot be built inside its stated
  lines is DROPPED and re-queued behind `I-23`, and the drop is reported.** Part 1 is the regression and it
  ships whatever else happens; the other four are here because they are in the same three files and cost one
  test run, and none of them is worth another week of `I-23` not existing (A-88 Part 11).
...
destinations and **100 %** against 50 large cities. The layer selects by administrative rank and
population; travel destinations are selected by **notability**. This increment changes the axis.
...
- **Built, in six parts, in this order. Part 1 is the generator and nothing downstream is written until it
  reports.**
  1. **`tools/gen-gazetteer.mjs` is rebuilt against GeoNames**, keeping every rule A-82 Part 2 gave it —
  ...
  2. **The selection rule, A-83 Part 3, verbatim**: class `P` or feature code `ISL`/`ISLS`, and
     ...
     `population ≥ 20,000`). *Languages* counts **distinct ISO-639 codes** in `alternateNamesV2`, one per
     (feature, language) however many spellings it has, ignoring `link`/`wkdt`/`iata`/`post`/`abbr`.
  ...
  4. **The representation and the shards** (A-83 Parts 4, 5 and 6). The fold is **not shipped** and is
     ...
     `packages/core/src/geo/gazetteer/meta.json` plus one JSON document per shard, split **while any
     exceeds 96 KiB**, keyed by token prefix, reached from a generated
     `packages/core/src/geo/gazetteerShards.gen.ts` mapping key → `() => import('./gazetteer/<key>.json',
     { with: { type: 'json' } })`. **Both mechanisms are verified and neither is assumed** — Node v22.22.2
     with no flags, and this repository's own Vite 8.2.2, which emits one lazy chunk per shard.
  5. **The loader and the pairing check** (A-83 Part 7). `@cairn/core/gazetteer` carries **exactly one**
     runtime symbol and it is renamed `GAZETTEER` → `loadGazetteerFor(query)`, returning `null` for a
     folded query under two characters. `searchGazetteer` stays **pure, synchronous and on `index.ts`**,
     and **throws a named programmer error when handed a shard the query does not belong to**. `opts.limit`
     is validated with `Number.isInteger` beside the existing `limit <= 0` (**R60-6**). The loader fetches
     `meta.json` **once** and **refuses a shard whose `$sourceSha256` does not match it**, which is the
     one hazard a global admin-1 dictionary buys and A-83 Part 4 says so.
  6. **The label rules** (A-83 Part 9) and the fold's docstring (A-83 Part 10). Labels are **distinct
     within a result set**: a colliding group gains ` (lat, lng)` at 2 dp. `gazetteer.ts:132-134`'s false
     sentence about U+0130 comes out and is replaced by what the 24-permutation measurement actually
     pins; `labelFor`'s docstring stops describing a fallback that cannot occur.
- **Not built, and named so nobody adds it.** **No `.tsx`, no `apps/web`, no `allowBare` entry.** **No
  record-shape change and no version movement.** **No fuzzy matching, no substring matching, no non-Latin
  name columns, no prefix trie** — A-82 Part 3 refuses each by name and A-83 keeps every refusal. **No
  second subpath.** **No cross-shard reference and no separate index** — A-83 Part 6 rules one search, one
  shard. **No coarsening below 4 dp.** **No `qa/` file, no `docs/design/`.**
- **User-visible outcome.** `node cli.ts cities hallstatt` returns **Hallstatt, Upper Austria, Austria**;
  so do `positano`, `zermatt`, `sintra`, `cesky krumlov`, `matera` and `carcassonne`. `obidos` returns
...
- **Verification.** Tagged per **How a criterion is written**. Every injected fault is run
  **red-before-green** and its measured output recorded.
  - **Coverage, and it is the criterion this increment exists for** `[stated]`: **`qa/r60-coverage.mjs` is
    re-run unmodified** and the travel hit rate is recorded. **The floor is 85 % on the travel set and
    100 % on the 50-city control**, and a control below 100 % is a **failed** ship gate, not a note —
    the control is what proves the dial did not trade away the easy case. The rate itself is reported as
    a measurement and no test pins it (rule 6). **N1, injected:** raise the language threshold to 12 →
    the travel rate falls below the floor and the run is recorded as red.
  - **The bytes, as ceilings not floors** (rule 4) `[stated]`: **no shard document exceeds 96 KiB**;
    the **total** committed size of `packages/core/src/geo/gazetteer/` does not exceed the generator's own
    reported figure, pasted, not rounded; every generated `.ts` file stays under the **1,048,576**-byte
    type-stripping ceiling. `0-gazetteerBudget.test.ts` keeps both of `0-countryBudget.test.ts`'s rules:
    it **never imports what it guards** (`statSync` only) and its numbers are measured, not chosen.
    **N2, injected:** raise the split budget so one shard exceeds 96 KiB → the per-shard ceiling fails
    naming the shard.
  - **One search, one shard, and the answer is the same as the whole corpus's** `[stated]`: for a named
    list of queries including **`york`** — the token-prefix case the sharding rule is hardest on — the
    result of searching the shard `loadGazetteerFor` returns is **identical, row for row and in order**,
    to searching a gazetteer assembled from **every** shard. **N3, injected:** shard a row by its first
    token only instead of by every token → the `york` equality fails, naming *New York*. **This is the
    fault that would silently make the search incomplete, and it is the one this design is most likely to
    commit.**
  - **The pairing is checked** `[stated]`: `searchGazetteer('york', <the shard for 'ha'>)` **throws** a
    named error rather than returning `[]`. **N4, injected:** delete the check → it returns `[]`, which is
    indistinguishable from a genuine miss, and the assertion fails.
    **And the two documents are checked against each other** `[stated]`: a shard whose `$sourceSha256`
    disagrees with `meta.json` is **refused by name**, not decoded with the wrong region dictionary.
    **N4b, injected:** hand-edit one shard's `$sourceSha256` → the loader throws naming the shard.
  - **The fold, and its ordering** `[stated]`: A-82 Part 3's twelve pairs, unchanged. **N5a, injected:**
    move `.toLowerCase()` below the substitution step → **exactly `Łódź` and `Đông Hà` fail and
    `İstanbul` passes**. **N5b, injected:** delete `.normalize('NFD')` → **exactly eight of the twelve
    fail.** Both outcomes are ceilings and both are A-83 Part 10's measured values. **The old N3 — *"move
    `.normalize('NFD')` above `.toLowerCase()` and the `İstanbul` case fails"* — is DELETED: measured, it
    fails nothing.**
  - **The fold is verified without being shipped** `[stated]`: a test decodes **every** shipped row and
  ...
  - **The bundle boundary** `[stated]`: the module graph reachable from `packages/core/src/index.ts`
    includes **neither** `geo/gazetteerShards.gen.ts` **nor** any file under `geo/gazetteer/`. **N9,
    injected:** re-export the shard map from `index.ts` → that test fails **and** `surface.test.ts`'s
    subpath set equality fails. **The main chunk may not grow by more than 2 kB** against the figure
    `I-22` recorded, and a main chunk that grew by the size of a shard is a **failed** ship gate.
  - **Determinism** `[stated]`: the generator run **twice** produces byte-identical output across
    **every** emitted file; `--audit-only` audits the committed bytes and fetches nothing; the ranking is
    still total, asserted over a shuffled copy of a shard's rows.
  - **The round trip compares structures** `[stated]`: the generator parses what it wrote and compares it
    field by field to what it built. **N10, injected:** put a `|` into one row's name and disable the
    delimiter refusal → the round trip fails naming the row. **Under the string comparison this replaces,
    it passed** (**R60-5**), and the report records both runs.
  - **The golden-scan exemption** `[stated]`: `test/cli.test.ts`'s A-56 scan exempts **exactly one** name,
    `gazetteer-probes.json`, and asserts positively that **every `centre` published in it matches a
    shipped row** (**R60-8**).
  - **Negative controls, which must stay green**: `fixtures/golden/core-*.json`, `countries.json`,
    `country-holes.json`, `forgiveness-drops.json`, `travel-stats.json` and the sample sha **all
    unchanged**; `SCHEMA_VERSION` and `SUMMARY_VERSION` **unmoved, asserted by name**;
    `0-countryBudget.test.ts` unmoved.
  - **Cost, measured and published as a measurement**: `node --test packages/core` wall time before and
...
- **Ship gate.** `npm test` green; `npm run typecheck` exit 0 on **both** projects (the door and illegal
  ...
  *Architecture / data model* above); the subpath set equality at **1**; **N1–N10 each run red-before-green with their measured output recorded**; the generator run
  ...
  `packages/core/src/geo/gazetteerShards.gen.ts` (new), `packages/core/src/geo/gazetteer/**` (generated),
  `packages/core/src/geo/gazetteer.gen.ts` (deleted), `packages/core/src/index.ts`,
  `packages/core/package.json`, `packages/core/test/gazetteer.test.ts`,
  `packages/core/test/0-gazetteerBudget.test.ts`, `packages/core/test/surface.test.ts`,
...
> **superseded** — see the banner on it. **Out-of-fence edit, DISCLOSED, recorded and NOT reverted:**
> `apps/web/src/ports/map.ts` (`MapBoundsLike` → `MapBounds`, five sites plus the import) — the unavoidable
> consequence of A-92 and correct. **⚠ CORRECTED AT REVISION 75 (R68-7): it WAS in the builder's report,
> twice, and it is NOT a third instance after R62-7/R64-7. The fence that forbade it was unsatisfiable
> alongside A-92 plus a clean typecheck, and that is my defect, not the builder's.** See the revision-74
> ledger bullet.
...
> **⚠ BUILT at `c202a89` + `7fb7901`; QA round 68 (`515e564`) returned SHIP on it — 0 blockers, 3 MAJOR,
> 7 MINOR, none of them this increment's own subject.** The sixteen refusals were re-derived
> **independently**, from A-93 Part 2's words, over the **pre-clause-4** corpus joined to the cached pinned
> dump, and agree with the committed golden **by GeoNames id in both directions**. `Antilles` and
> `Hispaniola` are gone; **R67-3 is FIXED**. Three document corrections the round owed are made in
> **revision 75**: this entry's class-`P` criterion (**R68-6**/KD-126, below), A-93 Part 2's `P(c)` join
> (**R68-2**/KD-125) and fault 3's arithmetic (**R68-5**) in `ARCHITECTURE.md`. **The builder was right
> three times in this arc** — KD-122, KD-124 and KD-125 — and each time the ruling changed.
> **`I-32` carries what round 68 found beside it, including the territory this increment's own corpus
> deletes for an unrelated reason.**
...
- **Verification.** Tagged per **How a criterion is written**, and **the ceilings are ceilings** (rule 4).
  ...
  - **The refusal set is the sixteen, by name and by id** `[stated]`: the golden's `'multi-country'` group
    is exactly A-93 Part 2's table — sixteen rows, each asserted by GeoNames id — and **no row refused
    under `reason: 'multi-country'` has feature class `P`**. *(⚠ CORRECTED AT REVISION 75 — QA **R68-6**,
    BUILD-NOTES **KD-126**, ARCHITECTURE §8.4 A-94 Part 7 item 6. This read *"no refused row of **any**
    reason has feature class `P`"*, which is **false and was false before this increment**:
    `Bantam Village` (`gn:x5yu`, `P/PPL`) is refused under `'bare-name'`. **A-93 Part 8 condition 1's own
    scoping — clause 4's matches — is correct, it is what the builder shipped, and the builder is upheld.**
    The wider form was built first, fired on the first run and wrote nothing, which is the guard working.
    Note that `I-32` makes `Bantam Village` ship, so the wider sentence would become true by a data change —
    which is exactly why it is corrected rather than left to be rescued.)* **N1, injected: drop the class
    restriction** → `Vatican City` disappears, the `vatican` probe becomes `NO MATCH`, the group goes
    **16 → 102** (revision 75, R68-5: `169` is A-89's pre-class count and the sovereign subtraction is still
    in place under this fault), and **three separate assertions redden**. This fault stays in the suite
    permanently: it is the version A-89 ruled and it is four characters.
...
#### I-32 — an abstention stops voting, the Cocos (Keeling) Islands come back, and the source log gets a witness the generator cannot rewrite (revision 75, `ARCHITECTURE.md` revision 72's §8.4 **A-94**; QA **R68-1** MAJOR, **R68-3** MAJOR, **R68-4**, **R68-9** and BUILD-NOTES **KD-127** ride along)

**Read §8.4 A-94 whole. Then A-84 Part 5, then A-89 Parts 4 and 5, then A-90 whole, then A-83 Part 9
clause 1. Then BUILD-NOTES KD-127. Nothing else** — **not A-93** (its predicate does not move; its sixteen
refusals are asserted unchanged, which is a golden read and not a ruling read), not A-91, not A-92, not
A-85…A-88, not §2 whole, not §4, not §10.

**Why it exists.** `node cli.ts cities "west island"` returns **no match**. It is the capital of the Cocos
(Keeling) Islands, and the whole territory ships **zero rows** — refused as bare names because three
abstentions outvoted two answers in the parent election. **That is a defect a user meets by typing a place
they have been to**, it is the exact deletion A-93 Part 3(a) argues against, and it predates both rulings.
Beside it, the append-only source log A-90 made the mechanism of a reviewed re-pin can be destroyed by an
**ordinary** regeneration.

**What it is NOT.** **No `.tsx`, no `apps/web` file of any kind, no `packages/client/src`.** *(Fence note,
per R68-7: if a ruling this increment implements makes an `apps/web` edit unavoidable, the ruling wins, the
edit is disclosed in the report, and the fence is my defect to fix — do not choose between them silently.)*
**No new source, no new pin, no fetch, no `--repin`** — `$sourceSha256` does **not** change, this being an
election change over unchanged bytes, and **a source-log entry appearing in this commit is a defect**.
**It changes no record shape**; `SCHEMA_VERSION` (5) and `SUMMARY_VERSION` (8) do not move, and
`GazetteerRow.countryCode` stays `CountryCode | null`. **It does not re-open A-93's clause 4**, whose
sixteen refusals must come out of the run unchanged.

- **Built, in four parts, in this order. Part 1 is the generator and nothing downstream is written until it
  reports the eight rows.**
  1. **A-94 Part 2, at the modal-parent election in `tools/gen-gazetteer.mjs` (`:886`, `:891-894`).** A row
     the layer neither contains nor places within the 0.05° tolerance **is not written into the vote**. The
     plurality is taken over answers only; a tie is broken by the **lowest ISO code**; a code whose every
     candidate row abstains has **no** parent and its rows ship `countryCode: null`. **The tolerance does
     not move and the parent stays per-code and modal** — a per-row parent is already an injected fault
     (`Saint-Georges` ships `BR`).
  2. **The election is published.** `fixtures/golden/gazetteer-parents.json` gains, per undrawable stated
     code, the full tally — every answer with its count **and the abstention count beside it**. `CC → AU
     (AU:2, abstain:3)` and `TK → NZ (NZ:1, abstain:2)` become facts a reviewer reads offline instead of
     lines that scrolled past in a five-minute run.
  3. **A-94 Parts 5 and 6 — the log and the two goldens.** `writeSourceLog` moves **off** the ordinary
     write path: the log is written **only** under `--repin`, is **never** re-seeded, and a run that finds
     it missing, empty or unparseable **stops and reports** (the bare `catch {}` at `:1922-1924` goes with
     it). The append-only witness becomes a **prefix literal in `packages/core/test/`** — the committed
     entries as `{source, sha256, fetched}`, asserted as a prefix of the file on disk — and the file's own
     `$what` stops claiming the `previousSha256` chain is that witness. `fixtures/golden/gazetteer-refusals.json`'s
     header gains **`I-31`'s four audit numbers** (KD-127). **`fixtures/golden/gazetteer-manifest.json`**
     is new: one sha256, row count and byte length per shipped corpus file, recomputed by a test — a
     **golden, not `meta.json`**, because 967 hashes must not be added to a file every client fetches.
  4. **R68-9 — `write()` stops destroying the artefact of record before it replaces it.** Write the corpus
     to a sibling temp directory and rename, or write-then-delete-stale; today `:1664-1672` `rmSync`s every
     `.json` and *then* writes 963 documents, so an interrupted run leaves an empty or partial corpus
     recoverable only from git.
- **Verification.** Tagged per **How a criterion is written**, and **the ceilings are ceilings** (rule 4).
  - **The territory is reachable** `[stated]`: `loadGazetteerFor` answers `west island` with
    **`gn:x5xz West Island`** and `bantam` with **`gn:x5yu Bantam Village`**, each labelled *"…, Australia"*,
    and all five `CC` rows — `gn:x5xz`, `gn:x5yu`, `gn:x5y5`, `gn:x5yj`, `gn:x5xw` — ship, **named
    individually by GeoNames id**, with `countryCode: 'AU'`. They are **absent** from the refusals golden,
    whose `'bare-name'` group goes **18 → 13**. **N1, injected: put the abstention back on the ballot** →
    all five assertions redden, naming all five, and the bare-name group returns to 18. **This fault is
    permanent** — it is the shipped behaviour of two revisions and it is one line.
  - **The tally is published, per code, with its abstentions** `[stated]`: `gazetteer-parents.json` carries
    twelve tallies; `CC` is `AU:2, abstain:3` and `TK` is `NZ:1, abstain:2`, **and no other code's tally
    changes**. **N2, injected: widen the tolerance to 0.06° instead of fixing the election** → `Cocos
    Islands` (0.1267°) still abstains, so the five-row assertion reddens, **and** the other codes' tallies
    move, so the no-other-tally-moved assertion reddens too — which is how a dial is made visible as a dial.
  - **The `null` arm's price is paid out loud** `[stated]`, A-94 Part 3: **`gn:4h85j Atafu Village`,
    `gn:2eefa Fale old settlement` and `gn:4h85h Nukunonu` ship `countryCode: 'NZ'`**, each keeping its name
    and region, and **no shipped row carries `countryCode: null`**. The arm is asserted **by its mechanism,
    not by a count of zero**: the 13 codeless rows still carry `reason: 'bare-name'` in the refusals
    golden, and the type is still `CountryCode | null`. **N3, injected: make a codeless row's abstention
    elect a parent anyway** → *"Lesser Antilles, France"* ships and the bare-name group shrinks below 13.
  - **A-93's sixteen are untouched** `[stated]`: the `'multi-country'` group is byte-identical to
    `7fb7901`'s — sixteen rows, same ids — and **no row refused under `'multi-country'` has feature class
    `P`** (the corrected wording; `Bantam Village` is no longer refused at all, which is why the criterion
    is scoped rather than rescued).
  - **The corpus moved by exactly eight rows** `[stated]`: **149,085 → 149,090**, and the row-level diff in
    this increment's commit contains **five added rows and three changed rows and nothing else**, each named
    by id (§0 position 11, sequencing rule 11). `$sourceSha256` is **byte-identical** to `7fb7901`'s.
  - **The source log cannot be re-seeded by an ordinary run** `[stated]`: delete the log, run the generator
    with **no** `--repin` and every checksum matching → it **stops and reports, writing nothing**, and the
    corpus on disk is unchanged. **N4, injected: restore the seed branch** → the prefix assertion in
    `packages/core/test/` reddens naming the first entry that moved. **N5, injected: truncate the log's
    tail** (drop the newest entry for one source) → the prefix assertion is **green**, and the second half —
    every source named in the corpus's `$sourceSha256` inputs has an entry whose `sha256` is the one in use
    — **reddens**. *(N5 exists because the `previousSha256` chain was green for exactly this, and a prefix
    test alone would be too.)*
  - **The artefact has a digest over its own bytes** `[stated]`: the manifest golden covers **every** file
    under `packages/core/src/geo/gazetteer/`, the test recomputes it from disk, and **N6, injected: a
    length-preserving edit to one shipped row's population** → the manifest test reddens **naming the
    shard**. *(The control from R68-4 stays: the same edit to the row's country code is caught by the
    `indexSays` cross-check. The manifest is a tripwire under A-90 clause 1's review property, not a
  ...
  - **An interrupted write leaves the corpus intact** `[stated]`: kill the generator between the delete and
    ...
    the directory is never observed empty reddens.
  - **Nothing else moved** `[stated]`: the parent translation's named outcomes unchanged
    (Fort-de-France/Basse-Terre/Dzaoudzi/St.-Benoît `FR`, Longyearbyen `NO`, Saint-Georges `FR`,
    `Antilles`/`Hispaniola` absent); every shipped row's code is `null` or one `COUNTRY_INDEX` draws, zero
    exceptions; the total order still total; the fold unchanged; `indexSays` still exact and summing; the
    byte and shard ceilings held; the shard/meta pairing check still throwing; **coverage re-measured off
    the rebuilt corpus through the real `loadGazetteerFor` and reported, travel and control both**.
- **One stop-and-report condition, and it is a NAMED SET** (§0 position 12 (a)/(c), sequencing rule 12).
  **The set this increment moves is eight rows: `gn:x5xz`, `gn:x5yu`, `gn:x5y5`, `gn:x5yj`, `gn:x5xw` begin
  shipping; `gn:4h85j`, `gn:2eefa`, `gn:4h85h` change `countryCode` `null → NZ`. If the corpus diff contains
  any added, removed or changed row outside those eight — or if A-93's sixteen differ in either direction —
  stop and report, writing nothing.**
- **Dependencies / blockers.** `I-31` (built, `7fb7901`, SHIP at `515e564`). **`I-32` is NOT a blocker for
  `I-30`** — it touches no `.tsx` and no door the picker calls. If it lands first the picker inherits a
  territory it would otherwise be missing; if it does not, `I-30` ships with a published five-row hole and
  says so, exactly as it would have with `I-31`'s two. **The picker is still fenced by one thing and it is
  the unresolved visual direction.**
- **Route: builder + breaker, mandatory** (delegation table: committed generated data). The generator,
...
not A-83 Part 3, not A-84, not A-85, not A-86, not A-87, not A-88, not A-89, **not A-93, not A-94**, not §2
whole, not §10. *(Revision 75: `I-32`, like `I-31`, changes no type, no door and no call a picker makes.
What it changes is which rows exist — five more, in `AU` — and a picker reads that through the same loader
either way.)*
...
- **Built.**
  ...
  5. **A-60 Part 6** *(revision 42, QA **R44-2**. Read *"owed by the repair pass, not yet built"* from
     revision 42 until revision 59, and it was accurate for all of it — the repair pass it was owed by
     landed before the item was written. **BUILT at `4b02206`**, found by the I-11 readiness check;
     `travelStats.ts` now computes `obsA`/`obsB` first and tests trigger 3 on them.)*. Two halves,
...
>
> ***BUILT at `9c16984`*** *(recorded at revision 59). Queued at revision 43 and dropped for eleven
> revisions by the ride-along clause in I-11's dependency bullet; found by revision 54's readiness check and
> closed by a builder. BUILD-NOTES **KD-81** is closed with it.*
...
- **Dependencies / blockers.** I-0 through I-10, **and I-12, I-12a, I-13, I-13a, I-13b, I-13c, I-13d, I-13e, I-13g, I-13h and I-13i** (revision 40,
  ...
  ***READINESS, AS OF REVISION 59 (2026-09-08): `I-11` IS READY. EVERY DEPENDENCY THIS BULLET NAMES IS
  BUILT AND ON `master`. ZERO REQUIRED INCREMENTS REMAIN UNBUILT AND THE FULL CHAIN MAY BE ORDERED.***
  The three that revision 55 still listed as required are each discharged in the sub-bullets below, with the
  commit that closed them: **`I-9` at `0e556a0`**, **`I-12a` item 5 at `4b02206`** and **`I-13a` at
  `9c16984`**. The two deferrals (`I-10`, `I-13f`) are **disclosed, not owed** — the gate certifies them.
  **How this line was produced, so the next reader can reproduce it rather than trust it**: every commit
  hash this bullet cites was checked with `git merge-base --is-ancestor <hash> master`, and for the two
  newest the *code* was read rather than the commit message — `packages/core/src/derive/travelStats.ts`
  computes `obsA`/`obsB` and `packages/core/test/photos.test.ts` emits its two `t.diagnostic` lines before
  their assertions. On this tree `npm run typecheck` exits 0 and `npm run test:tap` reports **1525 tests,
  1525 pass, 0 fail**. **The record of when each gap was found and when it was closed is kept below on
  purpose** — this document does not erase the story of a gap it found — but the current, actionable state
  is this paragraph and nothing below it supersedes it.
  ...
  ***Measured at revision 54, against `git log` on `master` rather than against this document's own status
  ...
  updated in place.** **Revision 59 re-ran that same measurement and all three of those are now built; the
  paragraph above carries the result and the sub-bullets below carry the discharge notes.** This bullet is
  ...
  - ***`I-9` is required and was not started at revision 54.*** Measured against `master`: no `Participant`
    ...
    ***Measured at revision 59:*** **`I-9a` is built (`17da01a`) and `I-9b` is built (`a2facc7`)**; the
    participants arc also carries round 52's repair pass (`20c1cd7`) and round 53's **SHIP** confirmation
    (`e77cded`). **`I-9c` is specified and NOT yet built** — no commit on `master` carries it — and that is
    **not a gap in this gate's readiness**, for the reason stated two sentences above: it is a queued
    cross-cutting fix, it blocks nothing here, and it may land on either side of the gate.
  ...
  - ***`I-12a` item 5 (§8.4 A-60 Part 6) was NOT BUILT when revision 54 measured it; it landed on `master`
    at `4b02206` and this sub-bullet is discharged.*** **What was found, kept because the shape of the miss
    is the lesson:** the entry had said *"owed by the repair pass, not yet built"* since revision 42 and
    that was accurate — but the repair pass it was owed by (`74a2762`) **landed before revision 42 wrote the
    item** (`598cd7f`), so nothing ever carried it. `packages/core/src/derive/travelStats.ts` computed
    `rawA`/`rawB` from the `??` substitutes and tested disjointness on them; A-60 Part 6.3 requires the
    **supplied** edges (`obsA`/`obsB` first, `rawA`/`rawB` last) and there was no `obsA` in the file. 6.2's
    owed test — the one that distinguishes the per-field `??` from the pair-wide reading — did not exist
    either, which is why the fork stayed live. The measurable consequence was a `null` `firstDay` beside a
    readable `lastDay` strictly before `a` printing `[a, a]`, a single day the traveller is provably
    elsewhere. **What is in the file now, read at revision 59 rather than taken from the commit message:**
    `obsA`/`obsB` are computed first, behind A-59's pair-wide unreadable gate; triggers 1 and 2 (`noEdge`)
    and trigger 3 (`disjoint`) read them, with Part 6.3's ray arms on strict `<`/`>` where only one edge is
    supplied and Part 2's test unchanged where both are; `rawA`/`rawB` are computed last, from the `??`
    substitutes, and are read by the clamp alone. Part 6.2's per-field `??` is upheld and now pinned, Part
    6.5's one-row table prints `[a, b]`, and `travelStats.test.ts` carries the increment's +13 tests
    including Part 6.6's four injected faults.
  - ***`I-13a` was NOT BUILT when revision 54 measured it — the ride-along clause above is what hid it — and
    it landed on `master` at `9c16984`; this sub-bullet is discharged.*** **What was found:** two commits
    opened `packages/core/test/photos.test.ts` after revision 43 created the increment — `b24b14c` (round
    45's fix pass) and `70b9ee6` (I-13b) — and **neither carried it**. In the file as then shipped, the size
    case reported the measured delta and the per-photo figure **only inside an assertion message, which
    speaks when it fails**; the 128-character bound had no fixture-scoped comment and no pointer to A-61
    Part 8 residue 1; and **BUILD-NOTES `KD-81` was still open and still marked *"Routed to the
    architect"***, which I-13a's own *Verification* bullet says it closes. **What is in the files now, read
    at revision 59:** the `20 photos cost kilobytes` case takes the `TestContext` and emits two
    `t.diagnostic` lines **before** their assertions, so a **passing** run publishes them — measured on this
    tree with `npm run test:tap`, `A-61 growth: 20 photos add 15354 B at toJSON indent 2 = 767.7 B/photo`
    and `A-61 payload: longest string in the document is 15 characters`; the `longest < 128` assertion
    carries the fixture-scoped comment naming A-61 Part 8 residue 1, the *"128 is a property of THIS
    FIXTURE"* statement and the *re-derive against the fixture rather than raise it to fit* instruction; and
    BUILD-NOTES **`KD-81`** carries a `RULED at architect revision 43 as §10 A-61, and CLOSED at ROADMAP
    I-13a` banner in the KD-82 house style, with its body left as the record of the routing. **The
    ride-along clause stays as written** — I-13a was genuinely orderable anywhere after I-13 — but the
    lesson is kept: it is the clause that let the item be dropped, and what caught it was measuring against
    `git log` rather than against this document's own cells.
  ...
  **Re-verified at revision 59**, hash by hash with `git merge-base --is-ancestor <hash> master`, and with
  the three revision-54 gaps' own commits added to the same check: `0e556a0` (I-9), `4b02206` (I-12a item 5)
  and `9c16984` (I-13a) are all on `master`. **Nothing this bullet names is missing.**
  ...
  **`I-13f` is correctly absent from this list and does not block this gate**, and the revision-49 framing
  ...
  both halves** — I-13f is not a screen, and what stood there when revision 54 measured was the three
  unbuilt increments above. **All three are built as of revision 59**, so what now stands between here and
  shippable is this gate's own chain and nothing else — the conclusion those blocks reached for the wrong
  reason and that is true today for a measured one. Correcting it is part of the board rebuild this increment's *Architecture / data model* bullet
...
- **Ship gate.** A manager verdict of **SHIP**. Nothing else counts as the phase being done. **And one
  ...
  **Revision 55 sharpens what "every increment named above" means**: it is every increment the bullet still
  ...
  (`I-10`, `I-13f`). ***That count is ZERO at revision 59: all three are built and on `master`, this
  precondition is SATISFIED, and the chain may be dispatched.*** A deferral this document states, with what it costs written down, is a thing the gate
...
10. **A finding about the strength of a guarantee does not gate a product increment** (revision 72, §0
   position 10, ARCHITECTURE §8.4 **A-88** Part 11, QA **R65-1** vs **R65-2**). Two things arrive from a
   breaker looking alike and they are not alike. **A defect** is a difference between what the product does
   and what it should do, over a population that can be reached — it is fixed, and it may gate. **A finding
   that a claim is broader than the mechanism maintaining it** is a defect in the *claim*: it is answered by
   making the mechanism reach when that is cheap and structural, or by narrowing the sentence, and it is a
   MINOR to the architect. Round 65 is the worked example: **R65-1** made a shipped sentence go vague for a
...
12. **A rule over committed data is scoped by a named set, and an increment that builds one stops on a
   ...
   **KD-124**). `I-29` is the worked example and it worked: the builder built the ruled predicate, measured
   ...
   repository. **(d)** *(revision 75, §0 position 12 (d), ARCHITECTURE §8.4 **A-94**, QA **R68-1**.)*
   **A rule that aggregates evidence says what a silent witness contributes, and the tally goes in a
   committed golden.** The parent election counted *"the layer has no opinion about this row"* as a vote for
   *"this row has no country"*; three silences outvoted two answers and **an entire inhabited territory left
   the product**, with the tally visible only in a five-minute run nobody can re-run offline. Routing
