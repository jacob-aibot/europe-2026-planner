...
> | **A-59 Part 6's four injected faults — all four run, all four RED, recorded** | (1) Gate always accepts (`isUnreadableDay → false`): **5 tests red**, the corrupt-row tests throw `invalid IsoDate` again. (2) Gate accepts nothing (`isUnreadableDay → v is present`): **12 tests red**, including `A-56: a city's firstVisit/lastVisit are ITS OWN days` — and the reference trip at that gate reads `unreadableCityDates = 6` with all six cities on `2026-08-07 → 2026-08-22`, which is A-59 Part 6 fault 2's stated expectation, matched exactly. **Both directions**, per A-34 Part 4. (3) Drop the increment: **5 red**, the dates still right and the *count* wrong, which is the whole reason the count is a field. (4) `unreadableRows` returns `[]` unconditionally: **3 red**, and the attribution test names no row. Two of my own beyond the four, on A-60's disjointness boundary, because a `<`/`<=` slip at either edge erases a real day: `rawB <= a` → **1 red** (a range ending exactly on `a`), `rawA >= b` → **2 red** (Split's genuine arrival day, and the reference-trip line). |
