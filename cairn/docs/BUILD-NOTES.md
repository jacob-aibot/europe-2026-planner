...
> 14 MINORs fixed**; the two left are named below with their reason. `R41-14` is the architect's
> and was not touched — `Refusal.tsx` and `WorldMap.tsx` keep the two-copy mechanism exactly as
> shipped, and the only thing that changed there is the **test** that guards it.
>
> **Scope: 6 files.** `apps/web/src/styles.css`, `apps/web/src/views/Profile.tsx`,
> `qa/i8b-render.mjs`, `qa/i8b-faults.sh`, `test/views.test.ts`, `qa/README.md`.
...
> | **R41-13 — the guard is now the equivalence check it was already described as** | Unchanged: `Refusal.tsx`, `WorldMap.tsx`, and the two-copy mechanism (**R41-14 is the architect's and this pass does not pre-empt it**). Changed: the test. It **extracts** every prose literal `Refusal.tsx` can render — string literals and JSX text runs — instead of listing three, and asserts each appears in `WorldMap.tsx`; and it asserts the `rowId` **branch** itself, which is the mutation that stayed green when the guard only counted sentences. Both of R41-13's green cases are now faults **25** and **26** in `qa/i8b-faults.sh` and both are RED. `bash qa/r41-refusal-drift.sh` builds a worktree at `HEAD`, so **it reports the finding as reproduced until this commit is the HEAD it checks out**; run it again after this commit and its (b) and (c) go RED. |
> | **The two MINORs deliberately NOT fixed, and why** | **R41-10** (`--ink-faint` as text at 2.63:1) — the breaker routed it *"to builder, **at whichever increment next opens the Library**"*, confirmed it pre-existing and byte-unchanged by I-8b, and stated it is not blocking this gate. It is `.tripcard__meta--dim` on the Library, not on this surface, and fixing an ink used across five surfaces inside a Profile repair is exactly the scope creep §5.6 exists to prevent. Still open; `qa/r41-shell.mjs` §F1 still reports it. **R41-14** — architect, by the round's own routing, and a parallel pass is on it. |
...
> | **Objection to the design — none new, and one restated** | Nothing in this pass was implemented against my judgement. The standing objection is the one the I-8b row already records and round 41 routed to the architect: **§5.5 and §5.6 disagree by one import statement.** I left the mechanical fence in place and only strengthened the test around it. The **second consequence** the breaker names in R41-14 — that `WorldMap.tsx` still wears `.statrow`'s three stat boxes which §5.3 declares replaced, so the same numbers speak two visual languages one tab apart — is real and is **not** fixable inside a zero-line-diff fence. It needs the architect's sentence first. |
