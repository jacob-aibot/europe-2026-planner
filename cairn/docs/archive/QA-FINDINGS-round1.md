> **Archived.** This is the original round-1 `QA-FINDINGS.md`, superseded in the working tree by
> round 2's file (`cairn/docs/QA-FINDINGS.md`) at commit `0903b9a`. It is kept here for provenance
> and debugging, not for routine reading — none of the four pipeline agents read this directory by
> default. Round 1's findings (`F-n`) are cross-referenced by id from round 2's file and from
> `BUILD-NOTES.md`'s `KD-n` entries; this is the original text those ids point back to.
>
> Recovered verbatim from git history: `git show 604c72f:cairn/docs/QA-FINDINGS.md`.
