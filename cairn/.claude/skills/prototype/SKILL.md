...
> **Vendored into Cairn, 2026-09-02 (architect revision 40) — read this fence first.** Adopted by
> **Jacob's 2026-09-02 visual-direction ruling**, which names it a mandatory design resource.
> **`cairn/docs/DESIGN.md` §1 and the reference board in `cairn/docs/design/REFERENCE-BOARD.md`
> outrank it**, and `DESIGN.md` **P6** is the motion budget that binds here: ≤ 200 ms under a
> finger, ≤ 300 ms for a sheet, named curves only, no bounce/elastic, no entrance or scroll-reveal
> motion, everything interruptible, everything off under `prefers-reduced-motion`. **This skill may
> not add a runtime dependency** — any library it recommends is an `ARCHITECTURE.md` §9 A-55
> decision, not this skill's. Sibling skills it cross-references that are **not** vendored:
> `improve-animations`, `find-animation-opportunities`, `animation-vocabulary`, `animate-expo`,
> `apple-design`, `write-swift`, `ask-sonner`, `pick-ui-library`; where one is named, the Cairn
> substitute is `DESIGN.md` §6 (rendered verification) or A-55 (library choices).
