...
> **Vendored into Cairn, 2026-09-01 — read this fence first.** `ARCHITECTURE.md` §9 **A-55**
> adopts this skill for **construction-time interaction and motion craft** and for nothing else.
> **`cairn/docs/DESIGN.md` §1 outranks it**, and `DESIGN.md` **P6** is the motion budget that binds
> here: ≤ 200 ms under a finger, ≤ 300 ms for a sheet, named curves only, no bounce/elastic, no
> entrance or scroll-reveal motion, everything interruptible, everything off under
> `prefers-reduced-motion`. **This skill may not add a runtime dependency** — any library it
> recommends (Motion, Sonner, Base UI, …) is an A-55 decision, not this skill's. Sibling skills it
> cross-references (`review-animations`, `improve-animations`, `find-animation-opportunities`,
> `animate-expo`, `pick-ui-library`, `RECIPES.md`) are **not vendored**; where one is named, the
> Cairn substitute is `DESIGN.md` §6 (rendered verification) or A-55 (library choices).
...
If the task needs a *component* rather than an animation — a toast, a drawer, a command menu, a dropdown — **stop and read `cairn/docs/ARCHITECTURE.md` §9 A-55**, which is where Cairn's library choices are ruled. (Upstream sends you to `pick-ui-library` here; that skill is deliberately not vendored — see this directory's README.) The underlying point stands and is why A-55 keeps the Radix/Base UI door open: hand-rolling a modal is how you end up with a `<div>` dropdown and no focus management.
