...
const CENSUS_CITY_FIELDS: Record<keyof City, true> = {
  // `pick` joins at ROADMAP I-22a (§8.4 **A-84** Part 3) — what a human picked, recorded whole.
  // It replaces `placeId`, which I-22 added and A-84 replaced: a bare id is a pointer nothing can
  // resolve, and this is a record.
  key: true, name: true, countryCode: true, centre: true, pick: true, order: true, meta: true,
};
  ...
  let t = createTrip(
    {
      ...
      // `pick` is POPULATED rather than declared in `DECLARED_NULLS`, and at I-22a the reasoning
      // is stronger than `bookingId`'s was: it is no longer a scalar but a THREE-FIELD RECORD, so
      // a null one hides a whole subtree from the census (§8.4 **A-84** Part 3). Its `centre`
      // equals the city's, which is what makes the pick LIVE rather than stale.
      cities: [{ key: SRC_CITY, name: 'Vienna', countryCode: 'AT', centre: VIENNA, pick: { rowId: 'ne:j64c4z', centre: { ...VIENNA }, countryCode: 'AT' }, order: 0, meta: { ...CITY_META } }],
    },
    CTX('src-'),
  );
  t = addPlace(t, {
  ...
  let t = createTrip(
    {
      ...
      cities: (cfg.cities ?? [{ key: TGT_CITY, name: cfg.city ?? 'Vienna', order: 0 }]).map((c) => ({
        ...
        key: c.key, name: c.name, countryCode: 'AT', centre: VIENNA,
        pick: { rowId: 'ne:j64c4z', centre: { ...VIENNA }, countryCode: 'AT' }, order: c.order,
        meta: { ...CITY_META },
      })),
    },
    CTX('tgt-'),
