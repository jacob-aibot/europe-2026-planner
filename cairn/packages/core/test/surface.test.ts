 ...
 *
 * So: one array, 73 entries, set equality both ways. (69 in revision 5; `reassertRetirements`
 * joins in revision 6 under §2.7 A-5; `lifecycle` joins in revision 10 under §8.1/§8.9,
 * Phase 2 I-1; `countryOf` and `COUNTRY_INDEX` join under §8.4 clause 1, Phase 2 I-5.)
...
/** §2.10, transcribed. Runtime symbols only — 73 of them, grouped as the section groups them. */
const THE_LIST = [
  // model (7)
  ...
  'geoCheck', 'GEO_LIMIT_KM', 'lifecycle', 'countryOf', 'COUNTRY_INDEX',
  // conflict (6)
...
test('§2.10 is 73 symbols, and the list in this file is exactly that long', () => {
  assert.equal(THE_LIST.length, 73, 'the transcribed list is no longer §2.10\'s stated size');
  assert.equal(new Set(THE_LIST).size, 73, 'the list has a duplicate');
});
