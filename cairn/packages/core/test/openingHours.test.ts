...
test('A-20 assertion 6: isOpeningHours is NOT on the public surface (§2.10 is 91, and none of them is this)', async () => {
  ...
  // **91 since ROADMAP I-35 added `ask`, `matchQuestion` and `askableQuestions` under §11.9** —
  // the trip Q&A capability, its recogniser and its menu. The recogniser is the symbol a language
  // model would one day replace; `ask` is the one it may never replace. The trigger tables, the
  // daypart windows, `resolveCite`, the per-intent resolvers and the renderer all stay internal.
  // The assertion
  ...
  assert.equal(Object.keys(core).length, 91);
