...
test('A-20 assertion 6: isOpeningHours is NOT on the public surface (§2.10 is 74, and none of them is this)', async () => {
  const core = await import('../src/index.ts');
  // 71 at revision 19; 73 since Phase 2 I-5 added `countryOf` and `COUNTRY_INDEX` under §8.4
  // clause 1; 74 since Phase 2 I-6 added `SUMMARY_VERSION` under §8.4 clause 3. The assertion
  ...
  assert.equal(Object.keys(core).length, 74);
