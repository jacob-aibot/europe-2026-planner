...
function summaryFor(id: string, revision: number): TripSummaryRow {
  return {
    ...
    cityCount: 0, dayCount: 2, stopCount: 0, poolCount: 0, placeCount: 0, revision,
    countryCodes: [], cities: [],
...
test('a merge cannot write using a stale StorageVersion', async () => {
  ...
  storage.load = async (id: string) => {
    ...
    if (!interleaved) {
      ...
      await storage.saveIfVersion(id, held, core.toJSON(third), core.tripSummary(third, core.COUNTRY_INDEX));
    }
    return out;
  };
