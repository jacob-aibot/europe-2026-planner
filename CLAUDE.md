...
- **"Add" means add it to the app.** When Jacob says to add something, put it in the HTML (`DAYS`/`CITY_PLACES`/`OPTIONAL`) so he can see it on the page — don't just answer in chat. Commit and push to the working branch without a second confirmation; only ask first if the day/city/placement is genuinely ambiguous. Merging to main is separate and still needs an explicit go-ahead each time ("push to prod" or equivalent) — pushing to the branch is not that.
