...
| **Tue Aug 18** | Prague → Budapest | Ryanair, dep ~07:30 — **time corrected by Jacob Aug 16 (was entered as 19:30)** | **I54C9A** ⚠️ |
...
⚠️ **IU1TUY and I54C9A came from Jacob verbally, not from an email**, and still haven't turned up in either inbox after exhaustive searching. Don't re-run that search, it's exhausted. **Important correction (Aug 6):** an earlier pass of this doc assumed I54C9A's date moved from Aug 18 to Aug 15 alongside the Smartwings reissue, since the two tickets share a booking-ref pattern (YZGDTS, found via `eticket@amadeus.com` — 16Jul2026 issue said Aug 18, 04Aug2026 reissue said Aug 15) and that seemed like the only way the connection still worked. **That assumption was wrong.** Jacob confirmed directly that Prague→Budapest is still Aug 18 — the date held. The two flights are three days apart, not connecting — Prague is a real 3-night stay, not a layover. Lesson: a shared-looking pattern between two unconfirmed data points is not confirmation of either one. Ask, don't infer, when it's this load-bearing.

**Second correction (Aug 16):** the time on that same leg was also wrong — the app had it as 7:30pm, entered that way after the Aug 6 exchange. Jacob corrected it directly: I54C9A departs **7:30am**, not 7:30pm. This flips the Aug 18 day from "easy last morning in Prague, evening flight" to "early flight, arrive Budapest mid-morning" — Aug 18's stops in the app are rebuilt around the 07:30 departure (early checkout, no time for Petřín Hill/a sit-down lunch). Jacob then asked to fill the newly-open Budapest afternoon: pulled St Stephen's Basilica and Shoes on the Danube Bank forward from Aug 20 (light, no-booking-needed, fits a day that started at 5am) — all `sug:true`, my draft, not his plan. Aug 20 retimed accordingly and is a bit lighter now.
...
### Aug 14 replanned — Krka by FlixBus + boat excursion, not self-drive

Dropped the self-drive/rental-car version of Aug 14 (needed a car nobody was planning to rent, and put the whole hike in peak August heat). Replaced with a round-trip FlixBus, Split ↔ Skradin — booking 338 441 5948, €37.47 total (€14.49 out, €22.98 back), seats 9C outbound / 18A return, route 942a. Boarding pass PDF sent by Jacob and committed at `tickets/flixbus-split-skradin-3384415948.pdf`, linked from both Aug 14 stops (`ticket:true`). Jacob also sent a CloudFront-hosted copy of the same ticket, but the network here couldn't reach that domain to verify it resolves — used the committed PDF instead per the usual rule (a file in the repo can't 404 or expire).
